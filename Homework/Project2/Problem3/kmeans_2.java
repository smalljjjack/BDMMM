...
public class kmeans_2 {
	...
	//set the parameters and creates random centroids and runs for the specified number of iterations/.
	
	public static void main(String[] args) throws Exception {
		if(args.length<4)
			System.out.println("Enter input parameters *input file* *output folder* *number of clusters* *maximum number of iterations* [*combiner*]");
		boolean combiner_flag=false;
		if(args.length==5){
			if(args[4].toLowerCase().trim()==("combiner"))
				combiner_flag=true;
		}
		...
		while (r_iter > 0) {
			...
			job.setJarByClass(kmeans_2.class);
			...
			if (job.waitForCompletion(true)) {
				...
				BufferedWriter buffer_writer = new BufferedWriter(
						new OutputStreamWriter(
								hdfs.create(new Path(args[1]+"/centroids"))));
