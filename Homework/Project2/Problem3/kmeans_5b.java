...
public class kmeans_5b {
	...
	public static class CentroidReducer extends
		...
		public void reduce(IntWritable key, Iterable<Point> values,
			...
			Point new_centroid = centroids_map.get(key.get());
			...
			if (cluster_size > 0) {
				...
				new_centroid = new Point(cent_x, cent_y);// +","+sum_x+","+sum_y+","+cluster_size;
			}
	...
	public static class FinalCentroidMapper extends
			Mapper<Object, Text, Text, Point> {
		HashMap<Integer, Point> centroids_map = new HashMap<Integer, Point>();
		...
		public void map(Object key, Text value, Context context)
			...
			context.write(new Text(point.toString()),
					centroids_map.get(closest_centroid));
		}
	...
	public static void main(String[] args) throws Exception {
		...
		while (iter_num <= r_iter) {
			...
			job.setJarByClass(kmeans_5b.class);
		...
		Job job2 = Job.getInstance(conf, "kmeans");
		job2.addCacheFile(centroids_filepath.toUri());
		job2.setJarByClass(kmeans_5b.class);
		job2.setMapperClass(FinalCentroidMapper.class);
		job2.setOutputKeyClass(Text.class);
		job2.setOutputValueClass(Point.class);
		FileInputFormat
				.addInputPath(
						job2,
						new Path(args[0]));
