...
public class kmeans_3 {
	...
	//set the parameters and creates random centroids and runs for the specified number of iterations or until the convergence is reached based on the mentioned threshold.
		
	public static void main(String[] args) throws Exception {
		if(args.length<5)
			System.out.println("Enter input parameters *input file* *output folder* *number of clusters* *maximum number of iterations* *threshold for convergence* [*combiner*] [*number of reducers*]");
		...
		if(args.length==7){
			num_reducers=Integer.parseInt(args[6]);
		}
		...
		while (iter_num <= r_iter) {
			...
			job.setJarByClass(kmeans_3.class);
			...
			if (job.waitForCompletion(true)) {
				...
				//reads the centroids from the current iteration.
